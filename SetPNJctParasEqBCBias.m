...
niLimit = round((3/8).*nx);
piLimit = round((5/8).*nx);



%linspace(4E16, 20E16, nx);

Nd = 4e16 * 1e6; % Const. 1/cm3 (100 cm/m)^3
Na = 1e16 * 1e6;
NetDoping = zeros(1,nx);
NetDoping(1:niLimit) = Nd;
NetDoping(piLimit:end) = -Na;
NetDoping((niLimit + 1):(piLimit - 1)) = linspace(Nd, -Na, length(NetDoping((niLimit + 1):(piLimit - 1))));
